/****************************************************************************
 * arch/arm/src/k5_j721e/k5_j721e_timerisr.c
 *
...
#include "hardware/k5_j721e_rti.h"
...
static int k5_j721e_timerisr(int irq, uint32_t *regs, void *arg)
{
  /* Clear the RTI Compare 0 interrupts */

  putreg32(RTI_INT0, K5_J721E_RTI_INTFLAG);
...
void up_timer_initialize(void)
{
  ...
  up_disable_irq(K5_J721E_REQ_RTICMP0);
  putreg32(0x0, K5_J721E_RTI_GCTRL);
  putreg32(RTI_ALLINTS, K5_J721E_RTI_CLEARINTENA);

  /* Configure RTICOMP0 register and the RTIUDCP0 Register to initialize with
   * the calculated compare value.
   */

  putreg32(RTI_CMP0, K5_J721E_RTI_COMP0);
  putreg32(RTI_CMP0, K5_J721E_RTI_UDCP0);

  /* Configure the FRC0CLK clock by setting the RTICPUC0 register to the
   * calculated value.
   */

  putreg32(RTI_CPUC0, K5_J721E_RTI_CPUC0);

  /* Initialize the free-running counter and the RTI up-counter */

  putreg32(0, K5_J721E_RTI_FRC0);
  putreg32(0, K5_J721E_RTI_UC0);

  /* Clear any pending interrupts */

  putreg32(RTI_ALLINTS, K5_J721E_RTI_INTFLAG);

  /* Enable the RTI Compare 0 interrupts (still disabled at the VIM) */

  putreg32(RTI_INT0, K5_J721E_RTI_SETINTENA);

  /* Enable counter 0 */

  putreg32(RTI_GCTRL_CNT0EN, K5_J721E_RTI_GCTRL);

  /* Attach the interrupt handler to the RTI Compare 0 interrupt */

  DEBUGVERIFY(irq_attach(K5_J721E_REQ_RTICMP0, (xcpt_t)k5_j721e_timerisr, NULL));

  /* Enable RTI compare 0 interrupts at the VIM */

  up_enable_irq(K5_J721E_REQ_RTICMP0);
}
