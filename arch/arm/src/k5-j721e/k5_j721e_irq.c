/****************************************************************************
 * arch/arm/src/k5_j721e/k5_j721e_irq.c
 *
...
#include "hardware/k5_j721e_vim.h"
#include "k5_j721e_gio.h"
#include "k5_j721e_esm.h"
#include "k5_j721e_irq.h"
...
/****************************************************************************
 * Name: k5_j721e_error_handler
 ****************************************************************************/

static void k5_j721e_error_handler(void)
{
  PANIC();
}
...
#define K5_J721E_VIM_FIRQPR3 (K5_J721E_VIM_BASE+0x001c)
#define K5_J721E_VIM_REQENASET3 (K5_J721E_VIM_BASE+0x003c)
#define K5_J721E_VIM_REQENACLR3 (K5_J721E_VIM_BASE+0x004c)
...
void up_irqinitialize(void)
{
  ...
  vimram = (uintptr_t *)K5_J721E_VIMRAM_BASE;
  for (i = 0; i < (K5_J721E_IRQ_NCHANNELS + 1); i++)
    {
      *vimram++ = (uintptr_t)k5_j721e_error_handler;
    }

  /* Set Fall-Back Address Parity Error Register (also not used) */

  putreg32((uint32_t)k5_j721e_error_handler, K5_J721E_VIM_FBPARERR);
  ...
  putreg32(0, K5_J721E_VIM_FIRQPR0);
  putreg32(0, K5_J721E_VIM_FIRQPR1);
  putreg32(0, K5_J721E_VIM_FIRQPR2);
#ifdef K5_J721E_VIM_FIRQPR3
  putreg32(0, K5_J721E_VIM_FIRQPR3);
#endif

  /* Disable all interrupts */

  putreg32(0xfffffffc, K5_J721E_VIM_REQENACLR0);
  putreg32(0xffffffff, K5_J721E_VIM_REQENACLR1);
  putreg32(0xffffffff, K5_J721E_VIM_REQENACLR2);
#ifdef K5_J721E_VIM_REQENACLR3
  putreg32(0xffffffff, K5_J721E_VIM_REQENACLR3);
#endif
  ...
  irq_attach(K5_J721E_REQ_ESMHIGH, k5_j721e_esm_interrupt, NULL);
  irq_attach(K5_J721E_REQ_ESMLO, k5_j721e_esm_interrupt, NULL);
  up_enable_irq(K5_J721E_REQ_ESMHIGH);
  up_enable_irq(K5_J721E_REQ_ESMLO);
...
uint32_t *arm_decodeirq(uint32_t *regs)
{
  ...
  vector = getreg32(K5_J721E_VIM_IRQINDEX) & VIM_IRQINDEX_MASK;
  if (vector > 0)
    {
...
uint32_t *arm_decodefiq(uint32_t *regs)
{
  ...
  vector = getreg32(K5_J721E_VIM_FIQINDEX) & VIM_FIQINDEX_MASK;
  if (vector > 0)
    {
      /* Dispatch the interrupt.  NOTE that the IRQ number is the vector
       * number offset by one to skip over the "phantom" vector.
...
void up_disable_irq(int channel)
{
  ...
  DEBUGASSERT(channel >= 0 && channel < K5_J721E_IRQ_NCHANNELS);
  ...
  regaddr  =  K5_J721E_VIM_REQENACLR(regndx);
...
void up_enable_irq(int channel)
{
  ...
  regaddr  = K5_J721E_VIM_FIRQPR(regndx);
  regval   = getreg32(regaddr);
  regval  &= ~bitmask;
  putreg32(regval, regaddr);
#endif
  ...
  regaddr  = K5_J721E_VIM_REQENASET(regndx);
  regval   = getreg32(regaddr);
  regval  |= bitmask;
  putreg32(regval, regaddr);
}
